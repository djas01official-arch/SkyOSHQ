...
- The web workspace loads the root `.env` for local monorepo commands. Credentials authentication is registered only when `NODE_ENV` is exactly `development` or `test`; it is deliberately unavailable in production and unknown runtimes. Production can register the built-in Auth.js Google OIDC provider only when both server-only `AUTH_GOOGLE_ID` and `AUTH_GOOGLE_SECRET` are non-blank. Unknown runtimes and production without both values register no sign-in provider.
- Production Google OIDC uses closed enrollment. A Google profile must have a verified email and an exact pre-provisioned `Account(provider="google", providerAccountId=<Google sub>)` binding to an active, non-deleted SkyOS `User`. A matching email never links or creates an actor, membership, role, or workspace access; Google `sub`, not email or `hd`, is the external identity key. Unknown, conflicting, inactive, or malformed Google identities fail closed before Auth.js reaches its user/account creation path.
- A trusted deployment operator may pre-provision a binding with `pnpm auth:google:bind`. The interactive command requests the operator SkyOS `User.id`, target SkyOS `User.id`, and exact Google OIDC `sub`; it does not accept email, does not expose a web route, does not store OAuth tokens, and does not echo the subject after success. It requires an active operator and target user plus `DATABASE_URL`; it is not a public enrollment mechanism.
- Google authentication security events use a dedicated append-only global identity audit history because pre-provisioning and rejected sign-ins may not have an organization. It records only a SHA-256 fingerprint of the external subject and allowlisted metadata; it never records OAuth codes, provider tokens, client secrets, or JWTs.
...
- The Prisma adapter `Account` model maps each external `(provider, providerAccountId)` identity to exactly one stable SkyOS user. The initial external provider decision and account-linking boundary are recorded in [ADR 0008](./architecture/decisions/0008-production-authentication-external-identity.md). Self-service linking, public registration, invitations, password recovery, MFA assurance, SSO, and SCIM remain out of scope.
...
The `/ai` workspace provides owner-scoped conversations backed by the grounded retrieval boundary. `AI_PROVIDER=local` selects the deterministic, network-free adapter only in development and tests. Production selects one server-owned execution backend for each deployment; ordinary users never choose a provider. OpenAI configuration remains:
...
Anthropic is the second first-class backend and uses the stateless Messages API. Claude Sonnet 5 is the current pinned default; Claude Sonnet 4.6 remains an approved pinned version for regression and future orchestration experiments:
...
Gemini is the third first-class backend. Its Developer transport uses the stateless Interactions API, while the explicit Vertex transport uses GenerateContent. Gemini 3.6 Flash is the approved stable model:
...
Provider keys must come from the production secret manager and must never use a `NEXT_PUBLIC_` prefix. Blank values, documented placeholders, unknown providers, and models outside each adapter's compiled allowlist fail closed. Production never falls back to the deterministic provider or silently changes providers. Non-production external execution requires a deliberately injected offline transport so automated tests cannot contact a real API. CI continues to use `AI_PROVIDER=local` and requires no external credential or paid request.

Providers are execution backends for the future SkyOS orchestration layer—not user-facing personalities or mutually exclusive product modes. The provider registry can retain multiple immutable provider/model/policy versions while today's application still resolves one server-configured current backend. A future orchestrator may send the same SkyOS-created grounded context to independent solver, critic, verifier, or synthesizer runs; it must keep separate `AiRun` records and may not let any provider broaden retrieval, authorization, or the citation allowlist.

The provider-neutral orchestration persistence and execution foundation is documented in [ADR 0006](./architecture/decisions/0006-ai-orchestration-foundation.md). Grounding now becomes one immutable, versioned `GroundedContext` before provider execution and reuses the existing append-only retrieval snapshot/citation rows. `AiOrchestration` can durably identify FAST, BALANCED, DEEP, or CRITICAL policy shapes and independently attributed candidate, critic, verifier, or synthesizer child runs. Existing Chat and Knowledge Actions remain single-provider; no production multi-provider execution, synthesis, routing, fallback, user-visible mode, or credits system is enabled.
...
- Message acceptance updates conversation recency/title and creates the processing run in one database transaction. SkyOS then persists the exact GroundedContext and citation allowlist before the external provider call. Provider execution cannot be database-atomic; success commits the assistant message and terminal telemetry together, while failure retains the immutable approved context and records only a safe failed-run state.
- Each successful run records provider/model/version, duration, optional input/cache-write/one-hour-cache-write/cached-input/output/reasoning/total token usage, a bounded provider request id, and a fixed-precision estimated USD cost when pricing is configured for that exact provider/model. When OpenAI, Anthropic, or Gemini supplies a valid request ID, SkyOS atomically records an append-only provider execution reference keyed by provider/type/value; this supports future exact cost-evidence ingestion without backfilling historical runs. Failed runs retain safe failure metadata and an optional bounded request id, but never invent usage or cost. Provider-supplied fabricated citation ids are ignored.
- A provider billing or usage endpoint is not automatically a valid `AiRunCostEvidence` source. Provider-originated records first pass a registered source-ID capability gate and only an authorized request-level source can enter trusted ingestion. Authoritative run evidence requires both an exact provider-issued monetary amount and a verified exact join to one durable provider execution identity. The currently verified OpenAI Organization Usage/Costs, Anthropic Admin Usage Report, and Google Cloud Billing standard/detailed exports are aggregate-only and are deliberately blocked before normalization, mapping, or persistence; SkyOS does not proportionally allocate them or reprice tokens. The existing fixture/input-only ingestion adapters remain valid only for trusted callers that have independently verified both required properties.
- Gemini defaults to the API-key Gemini Developer Interactions transport. Vertex is an explicit opt-in only (`GEMINI_TRANSPORT=vertex`, `GOOGLE_CLOUD_PROJECT`, and `GOOGLE_CLOUD_LOCATION`): Developer uses `interactions.create`, while Vertex uses `models.generateContent` for the approved `gemini-3.6-flash` model. Vertex attaches only the lossless `skyos_run=run-<AiRun UUID>` correlation label in the GenerateContent configuration, derived from the persisted provider-attempt run. Developer requests receive no label. The label is distinct from a provider `REQUEST_ID`, carries no user or content data, and does not authorize Google Cloud Billing exports as `AiRunCostEvidence`; no billing ingestion exists and financial cost, credits, adjustments, and invoice semantics remain unresolved.
...
- OpenAI calls use the Responses API with `store: false`, strict JSON Schema output, a 1,200-token output cap, no tools or hosted retrieval, and no provider-hosted conversation state. Normal chat uses `{ answer, citationIds }`; Knowledge AI actions use action-specific schemas that preserve supported citations and nullable explicit-only fields before formatting canonical readable assistant text. SkyOS disables SDK retries and owns at most two transient retries within one 45-second aggregate deadline.
- Anthropic calls use `POST /v1/messages` through the official TypeScript SDK, stable `output_config.format` JSON Schema, standard-only service tier, and no cache control, tools, containers, hosted retrieval, or provider-native citation authority. The request omits `inference_geo` so the Anthropic workspace's allowed-geo policy and default remain authoritative; global standard cost is estimated only when the response reports `global`, while US-only or missing geo remains unknown. SkyOS sends no sampling or manual-thinking configuration. Sonnet 5 keeps its default adaptive thinking with a hard 16,000-token combined thinking/response ceiling; Sonnet 4.6 retains the 1,200-token response ceiling. Both retain the same 2,000-character parsed-output bound. SkyOS disables SDK retries and applies the same two-retry/45-second aggregate policy. Anthropic `input_tokens` excludes cache creation and reads, so the adapter derives provider-neutral total input from all three official fields and retains the one-hour subset when the provider supplies its TTL breakdown.
- Gemini Developer calls the GA Interactions API through the official `@google/genai` SDK with `store: false` and one top-level JSON-schema `response_format`. Vertex calls `models.generateContent` with the same bounded serialized input, system instruction, JSON-schema response contract, default thinking behavior, and 16,000-token output ceiling. Neither transport uses conversation reuse, tools, hosted retrieval, native grounding, sampling controls, or thought summaries. Provider schemas drop only unsupported string-length keywords; the full canonical SkyOS schema validates every returned JSON result. The safe interaction or Vertex response id is retained when valid. Input, cached-input, visible-output, thought, and derived-total tokens remain separate; provider thought content is neither requested nor persisted.
- Model pricing is centralized in `services/ai/language-model-pricing.ts`. Standard GPT-5.6 Terra requests use the dated catalog rates; requests with more than 272,000 input tokens apply 2x uncached-input and 1.5x output pricing to the full request (272,000 is still standard pricing). Current official documentation does not establish how that threshold changes cached-read or cache-write rates, so a long-context run containing either category deliberately leaves `estimatedCostUsd` null instead of guessing. Claude Sonnet 4.6 global standard processing uses the catalog's verified `$3` input, `$3.75` five-minute cache write, `$6` one-hour cache write, `$0.30` cache read, and `$15` output rates per million tokens. Claude Sonnet 5 uses its documented promotional rates through 2026-08-31 UTC (`$2`/`$2.50`/`$4`/`$0.20`/`$10`) and standard rates from 2026-09-01 UTC (`$3`/`$3.75`/`$6`/`$0.30`/`$15`). Gemini 3.6 Flash Standard pricing verified on 2026-08-14 is `$1.50` input, `$0.15` cached input, and `$7.50` output including thought tokens per million; visible output and separately reported thoughts are therefore each charged once at the output rate. SkyOS creates no explicit Gemini context cache and estimates no cache-storage charge. The immutable `AiRun.createdAt` selects the effective catalog entry. Unknown or ambiguous usage/pricing remains null rather than guessed.
- Workspace owners and admins can inspect compact workspace-wide telemetry at `/ai/usage`. The server requires both `ai.use` and `workspace.members.read`, scopes every aggregate and recent-run query to the effective selected workspace, and permits requester identity only under that membership-directory grant. Runs are bucketed by `AiRun.createdAt` using inclusive UTC starts and exclusive UTC ends because tenant-configurable time zones do not yet exist. Estimated cost totals exclude null costs and report the excluded count; token totals exclude unreported nullable fields and disclose incomplete-usage runs.
- Active Knowledge document and version pages expose summarize, action-item extraction, risk identification, and key-decision extraction to users with both `knowledge.read` and `ai.use`. Each action bypasses workspace search, pins one immutable `KnowledgeDocumentVersion` on its normal private conversation/run, packages only that version as direct citation-bearing untrusted context, and records normal usage/cost telemetry. Direct actions accept at most 8,000 source characters and fail safely rather than silently summarizing a partial document. Owners, due dates, and rationales remain null unless explicitly stated; empty supported result sets render an explicit no-results message. Results stay on the originating Knowledge page, and retry reuses the pinned source version.
