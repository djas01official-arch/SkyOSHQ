...
- Model pricing is centralized in `services/ai/language-model-pricing.ts`. Standard GPT-5.6 Terra requests use the dated catalog rates; requests with more than 272,000 input tokens apply 2x uncached-input and 1.5x output pricing to the full request (272,000 is still standard pricing). Current official documentation does not establish how that threshold changes cached-read or cache-write rates, so a long-context run containing either category deliberately leaves `estimatedCostUsd` null instead of guessing. Unknown provider/model pricing behaves the same way while preserving valid usage telemetry; changing a rate or rule requires updating and re-verifying that single catalog entry.
