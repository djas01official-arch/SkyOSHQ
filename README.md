...
Gemini is the third first-class backend and uses the stateless Interactions API. Gemini 3.6 Flash is the approved stable model:
...
- Gemini defaults to the API-key Gemini Developer Interactions transport. Vertex is an explicit opt-in only (`GEMINI_TRANSPORT=vertex`, `GOOGLE_CLOUD_PROJECT`, and `GOOGLE_CLOUD_LOCATION`); it keeps the same Interactions API and attaches only the lossless `skyos_run=run-<AiRun UUID>` correlation label derived from the persisted provider-attempt run. Developer requests receive no label. The label is distinct from a provider `REQUEST_ID`, carries no user or content data, and does not authorize Google Cloud Billing exports as `AiRunCostEvidence`; no billing ingestion exists and financial cost, credits, adjustments, and invoice semantics remain unresolved.
...
- Gemini calls the GA Interactions API through the official `@google/genai` SDK with `store: false`, one top-level JSON-schema `response_format`, and no `previous_interaction_id`, background work, tools, hosted retrieval, native grounding, sampling controls, or thought summaries. SkyOS supplies one bounded JSON-encoded conversation/context input, keeps Gemini's default thinking behavior, and applies a 16,000-token combined ceiling plus the canonical 2,000-character parsed-output bound. Provider schemas drop only unsupported string-length keywords; the full canonical SkyOS schema validates the returned JSON. The safe interaction id is retained when valid. Input, cached-input, visible-output, thought, and derived-total tokens remain separate; provider thought content is neither requested nor persisted.
