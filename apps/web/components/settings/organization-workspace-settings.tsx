...
export function OrganizationWorkspaceSettings({ context }: OrganizationWorkspaceSettingsProps) {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        description="Review trusted tenant context and create the minimal organization boundaries used by SkyOS."
        ...
        <div className="space-y-6">
          <Card>
            ...
            {context?.activeOrganization ? (
              <dl className="mt-6 grid gap-4 sm:grid-cols-3">
                ...
                <div>
                  ...
                  <dd className="mt-1 font-mono text-sm text-foreground">
                    {context.activeOrganization.slug}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  ...
                  <dd className="mt-1">
                    <Badge tone="accent">{roleLabel(context.activeOrganization.role)}</Badge>
                  </dd>
                </div>
              </dl>
            ) : (
              <div className="mt-6">
                <EmptyState
                  description="Create an organization to establish your first tenant boundary."
                  icon="grid"
            ...
            {context?.activeWorkspace ? (
              <dl className="mt-6 grid gap-4 sm:grid-cols-3">
                ...
                <div>
                  ...
                  <dd className="mt-1">
                    <Badge tone="success">{roleLabel(context.activeWorkspace.role)}</Badge>
                  </dd>
                </div>
              </dl>
            ) : (
