...
75. Accepting a user message, updating conversation recency/title, and creating its processing run commit atomically before external execution. A run captures application-selected provider/model/version, timing, bounded usage (including a provider-neutral total cache-write count and an optional one-hour cache-write subset), optional safe provider request identity, status, and safe failure category. Successful runs may record fixed-precision estimated cost only from a dated application-owned price for their exact provider/model and reported cache semantics; unknown pricing remains null. Failed runs never invent usage or cost. Only `processing` may transition once to `succeeded` or `failed`; concurrent active runs for one user message are rejected.
