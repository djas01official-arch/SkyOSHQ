...
75. Accepting a user message, updating conversation recency/title, and creating its processing run commit atomically before external execution. A run captures application-selected provider/model/version, timing, bounded usage (including provider-neutral cache-write, cached-input, visible-output, optional reasoning/thought, and derived-total counts), optional safe provider request identity, status, and safe failure category. Provider reasoning content is never persisted. Successful runs may record fixed-precision estimated cost only from a dated application-owned price for their exact provider/model and reported cache/reasoning semantics; unknown pricing remains null. Failed runs never invent usage or cost. Only `processing` may transition once to `succeeded` or `failed`; concurrent active runs for one user message are rejected.
...
85. A non-null assignee must have active user, organization-membership, and workspace-membership state in the task's exact workspace at assignment time. Later loss of access does not erase historical attribution, does not grant access, and prevents reselection until membership becomes effective.
86. Archived tasks are excluded from normal lists and detail reads. Task restore, hard delete, recurring work, comments, dependencies, subtasks, attachments, notifications, labels, custom fields, automations, reminders, and calendar integrations are outside Tasks MVP v1.
87. Normal task lists use bounded forward keyset pages in deterministic `status ASC`, `dueAt ASC NULLS LAST`, `updatedAt DESC`, `id ASC` order. The default page size is 25 and the maximum is 100. The versioned cursor contains that complete tuple plus the trusted workspace id, is validated only after effective `tasks.read`, and never grants authority. Malformed, noncanonical, and cross-workspace cursors fail safely without disclosing task existence.
88. A task due date is a calendar date, not a timestamp. Persistence and presentation must preserve the selected date without local-time-zone drift.
89. Task creation, update, and archive append immutable success audit events in the same transaction as the mutation. Validation and authorization denial append no success event; an audit insertion failure rolls back the protected write.
90. Task update and archive use `updatedAt` as a canonical optimistic-concurrency token. Authorization, input validation, and assignee validation precede an atomic compare-and-swap constrained by trusted task and workspace identity plus active state. A malformed or stale token changes no row and emits no success event; successful writes advance `updatedAt` by at least one millisecond. Client tokens never grant authority, and conflict handling does not merge changes or disclose cross-workspace task existence.
