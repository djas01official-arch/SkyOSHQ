...
81. Every task belongs to exactly one workspace. Its `id`, `workspaceId`, and `createdByUserId` cannot be reassigned after creation, and creator attribution grants no authority.
82. Listing or reading tasks requires effective `tasks.read`; creating, updating, assigning, unassigning, changing status or priority, and archiving requires effective `tasks.write`. All checks derive workspace scope from trusted server context and the persisted task, never a client-supplied workspace id.
83. A non-null assignee must have active user, organization-membership, and workspace-membership state in the task's exact workspace at assignment time. Later loss of access does not erase historical attribution, does not grant access, and prevents reselection until membership becomes effective.
84. Archived tasks are excluded from normal lists and detail reads. Task restore, hard delete, recurring work, comments, dependencies, subtasks, attachments, notifications, labels, custom fields, automations, reminders, and calendar integrations are outside Tasks MVP v1.
85. Normal task lists use bounded forward keyset pages in deterministic `status ASC`, `dueAt ASC NULLS LAST`, `updatedAt DESC`, `id ASC` order. The default page size is 25 and the maximum is 100. The versioned cursor contains that complete tuple plus the trusted workspace id, is validated only after effective `tasks.read`, and never grants authority. Malformed, noncanonical, and cross-workspace cursors fail safely without disclosing task existence.
86. A task due date is a calendar date, not a timestamp. Persistence and presentation must preserve the selected date without local-time-zone drift.
87. Task creation, update, and archive append immutable success audit events in the same transaction as the mutation. Validation and authorization denial append no success event; an audit insertion failure rolls back the protected write.
88. Task update and archive use `updatedAt` as a canonical optimistic-concurrency token. Authorization, input validation, and assignee validation precede an atomic compare-and-swap constrained by trusted task and workspace identity plus active state. A malformed or stale token changes no row and emits no success audit event; successful writes advance `updatedAt` by at least one millisecond. Client tokens never grant authority, and conflict handling does not merge changes or disclose cross-workspace task existence.
