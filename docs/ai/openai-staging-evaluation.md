...
- The model page currently lists standard text pricing of **$2.50 per 1M input tokens**, **$0.25 per 1M cached input tokens**, and **$15.00 per 1M output tokens**, with cache writes billed at 1.25 times the uncached input rate. The snapshot was verified on 2026-08-13 and must be re-verified before each controlled run. Planning assumes uncached input; actual-cost calculation uses bounded cache-write and cached-input breakdowns when the provider returns them.
