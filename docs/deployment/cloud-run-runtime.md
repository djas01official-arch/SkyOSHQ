...
| Reconciliation | `DATABASE_URL`, `KNOWLEDGE_STORAGE_PROVIDER=gcs`, `KNOWLEDGE_GCS_BUCKET`, and the pinned embedding provider identity used for safe pipeline repair. It remains report-only by default; its bucket role permits bounded/paginated object reads and listing, never mutation.                                                                  |
