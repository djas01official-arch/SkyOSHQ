locals {
  ...
  web_active_secret_env_names = var.enable_web_service ? toset([
    for secret_name in local.web_configured_secret_env_names : secret_name
    if secret_name != "ANTHROPIC_API_KEY" || !local.anthropic_web_wif_any_configured
  ]) : toset([])
}
