...
test('publisher keeps the database password out of gcloud command arguments', () => {
  ...
  const runner: GcloudRunner = (args, input) => {
    ...
    if (args.includes('add')) {
      return JSON.stringify({
        name: 'projects/skyos-test-project/secrets/skyos-np-database-url/versions/7',
      });
    }

    return JSON.stringify({ name: 'projects/skyos-test-project/secrets/example' });
  };
  ...
  assert.equal(
    calls.every(({ args }) => !args.some((argument) => argument.includes('value(name)'))),
    true,
  );
